...
{	
	...
	NSDictionary *config = [NSDictionary dictionaryWithContentsOfFile: kConfigPath];
	NSMutableDictionary	*colorInt = [config objectForKey:[[[[cell message] account] firstEmailAddress] lowercaseString] ];
	id *wantAl = [config objectForKey:@"main_mailbox_color_aliases"];
	int	optionInt = [[config objectForKey:@"mail_label_style"] intValue];
	
	if ([[cell subviews] indexOfObject:(UIView*)[cell viewWithTag:333]] != 0 && optionInt!=6) 
	{
		[cell sendSubviewToBack:(UIView*)[cell viewWithTag:333]];
		
	} else if (optionInt == 6)
	{
		[cell bringSubviewToFront:(UIView*)[cell viewWithTag:333]];	
	}
	...
	{
		colorInt = nil;
		id to = [[cell message] valueForKeyPath:@"_to"];
		id cc = [[cell message] valueForKeyPath:@"_cc"];
		...
		{
			colorInt = [config objectForKey:[[[[cell message] account] firstEmailAddress] lowercaseString] ];
		}
		
	}
	
	
	UIView *extra = (UIView*)[cell viewWithTag:333];
	id y;
	switch (optionInt)
	{
		case 4:
			y = [cell valueForKeyPath:@"_dateLabel"];
			...
			break;
		default:
			extra = (UIView*)[cell viewWithTag:333];
			break;
	}
